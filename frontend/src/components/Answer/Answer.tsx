...
import { Checkbox, DefaultButton, Dialog, FontIcon, Stack, Text, IconButton, TooltipHost} from '@fluentui/react'
...
interface Props {
  answer: AskResponse & { modelInfo?: string; knowledgeBaseInfo?: string };  
        ...
        <Stack horizontal className={styles.answerFooter}>
          ...
          <Stack.Item className={styles.answerDisclaimerContainer}>
            <Stack horizontal verticalAlign="center" horizontalAlign="space-between" styles={{ root: { width: '100%' } }}>
              ...
              <TooltipHost
                content={`Model: ${answer.modelInfo || 'Unknown'}, Knowledge Base: ${answer.knowledgeBaseInfo || 'Unknown'}`}
                id={`info-tooltip-${answer.message_id}`}
                calloutProps={{ gapSpace: 0 }}
              >
                <IconButton
                  iconProps={{ iconName: 'Info' }}
                  aria-label="Model and Knowledge Base Information"
                  styles={{
                    root: { color: '#0078D4' }
                  }}
                />
